...
# ======================================
INTERVAL_RESULT_PRINT = 25
INTERVAL_MODEL_SAVE = 25
NUM_EPISODES = 1000
# 
steps_done = 0

BEST_SAVE_FROM_EP = 300
best_score_since_threshold = float('-inf')
...
class RewardWrapper():
    ...
    def step(self, action):
        ...
        angle_reward = 2.0 * np.cos(theta)
        theta_dot_penalty = 0.0005 * (theta_dot ** 2)

        if abs(x) > 1.5:
            out_of_bound_penalty = 25 * (abs(x) - 1.5) ** 2
        else:
            out_of_bound_penalty = 0.0

        position_penalty = 1.0 * (x ** 2)
        x_dot_penalty = 0.001     * (x_dot ** 2)

        reward = angle_reward - theta_dot_penalty - out_of_bound_penalty - position_penalty - x_dot_penalty

        abs_theta = abs(theta)

        if abs_theta < 0.15 and abs(theta_dot) < 0.5:
            reward += 10.0

        if abs_theta < 0.15 and abs(theta_dot) < 0.5 and abs(x) < 0.5:
            reward += 15.0

        if abs_theta < 0.1 and abs(theta_dot) < 0.3 and abs(x) < 0.1:
            reward += 30.0
    ...
    for i_episode in range(NUM_EPISODES):
        ...
        for t in count():
            ...
            if done:
                ...
                # --- Add: best-score checkpointing from ep >= BEST_SAVE_FROM_EP ---
                if (i_episode + 1) >= BEST_SAVE_FROM_EP:
                    curr_score = episode_scores[-1]
                    if curr_score > best_score_since_threshold:
                        best_score_since_threshold = curr_score
                        torch.save(
                            policy_net.state_dict(),
                            f"{current_dir}/{result_folder_path}/policy_net_best_ep{i_episode+1}_score{int(curr_score)}.pth"
                        )
                        print(f"[BEST SAVE] ep {i_episode+1} | new best score {curr_score:.2f} -> model saved")
                break

        if i_episode % INTERVAL_RESULT_PRINT == 0:
            print(
                f"n_episode :{i_episode}, score : {np.mean(episode_scores[-INTERVAL_RESULT_PRINT:]):.2f}, n_buffer : {memory.__len__()}, eps : {epsilon:.3f}")

        if (i_episode + 1) % INTERVAL_MODEL_SAVE == 0:
            plot_durations(episode_durations, save_plot=True)  # .png 저장
            plot_scores(episode_scores, save_plot=True)  # .png 저장
            # save_model(policy_net, i_episode + 1)  # .pth 저장
